...
void Initialize() {
	...
	GPIOPinsInit(IP2_PIN,  GPIO_MODE_OUTPUT_PP, GPIO_SPEED_FREQ_MEDIUM,GPIO_PULLUP);
	...
	CANxInit(&hcan1, CAN_FILTER_FIFO0, CAN_FILTERSCALE_32BIT, 0, 0, 12, CAN_500KHz);	// RNS & VESC
...
//	PWMChannelConfig(&htim1, TIM_CHANNEL_3, IP1_PIN);
	PWMChannelConfig(&htim1, TIM_CHANNEL_3, IP2_PIN); // hspm2
